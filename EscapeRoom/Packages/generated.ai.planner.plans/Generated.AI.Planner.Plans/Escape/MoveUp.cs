...
{
    [BurstCompile]
    struct MoveUp : IJobParallelForDefer
    {
        ...
        internal MoveUp(Guid guid, NativeList<StateEntityKey> statesToExpand, StateDataContext stateDataContext)
        {
            ActionGuid = guid;
        ...
        public void Execute(int jobIndex)
        {
            ...
            var transitionInfo = new NativeArray<MoveUpFixupReference>(argumentPermutations.Length, Allocator.Temp);
            for (var i = 0; i < argumentPermutations.Length; i++)
            {
                transitionInfo[i] = new MoveUpFixupReference { TransitionInfo = ApplyEffects(argumentPermutations[i], stateEntityKey) };
            }

            // fixups
            var stateEntity = stateEntityKey.Entity;
            var fixupBuffer = m_StateDataContext.EntityCommandBuffer.AddBuffer<MoveUpFixupReference>(jobIndex, stateEntity);
    ...
    public struct MoveUpFixupReference : IBufferElementData
    {
