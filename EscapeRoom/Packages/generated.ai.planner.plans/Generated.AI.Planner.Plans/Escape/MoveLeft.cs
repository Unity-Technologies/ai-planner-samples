...
{
    [BurstCompile]
    struct MoveLeft : IJobParallelForDefer
    {
        ...
        internal MoveLeft(Guid guid, NativeList<StateEntityKey> statesToExpand, StateDataContext stateDataContext)
        {
            ActionGuid = guid;
        ...
        {
            var CharacterFilter = new NativeArray<ComponentType>(1, Allocator.Temp){[0] = ComponentType.ReadWrite<Generated.AI.Planner.StateRepresentation.Character>(),  };
            var ToFilter = new NativeArray<ComponentType>(3, Allocator.Temp){[0] = ComponentType.ReadWrite<Generated.AI.Planner.StateRepresentation.Waypoint>(), [1] = ComponentType.Exclude<Generated.AI.Planner.StateRepresentation.ActivationLock>(), [2] = ComponentType.Exclude<Generated.AI.Planner.StateRepresentation.KeyLock>(),  };
            ...
            for (int i1 = 0; i1 < ToObjectIndices.Length; i1++)
            {
                ...
                if (!(WaypointBuffer[ToObject.WaypointIndex].Right == CharacterBuffer[CharacterObject.CharacterIndex].Waypoint))
                    continue;
        ...
        public void Execute(int jobIndex)
        {
            ...
            var transitionInfo = new NativeArray<MoveLeftFixupReference>(argumentPermutations.Length, Allocator.Temp);
            for (var i = 0; i < argumentPermutations.Length; i++)
            {
                transitionInfo[i] = new MoveLeftFixupReference { TransitionInfo = ApplyEffects(argumentPermutations[i], stateEntityKey) };
            }

            // fixups
            var stateEntity = stateEntityKey.Entity;
            var fixupBuffer = m_StateDataContext.EntityCommandBuffer.AddBuffer<MoveLeftFixupReference>(jobIndex, stateEntity);
    ...
    public struct MoveLeftFixupReference : IBufferElementData
    {
