...
namespace AI.Planner.Actions.Match3Plan
{
    ...
    struct SwapRight : IJobParallelForDefer
    {
        ...
        {
            var GameFilter = new NativeArray<ComponentType>(1, Allocator.Temp){[0] = ComponentType.ReadWrite<AI.Planner.Domains.Game>(),  };
            var SourceFilter = new NativeArray<ComponentType>(3, Allocator.Temp){[0] = ComponentType.ReadWrite<AI.Planner.Domains.Coordinate>(),[1] = ComponentType.ReadWrite<AI.Planner.Domains.Cell>(), [2] = ComponentType.Exclude<AI.Planner.Domains.Blocker>(),  };
            var TargetFilter = new NativeArray<ComponentType>(3, Allocator.Temp){[0] = ComponentType.ReadWrite<AI.Planner.Domains.Coordinate>(),[1] = ComponentType.ReadWrite<AI.Planner.Domains.Cell>(), [2] = ComponentType.Exclude<AI.Planner.Domains.Blocker>(),  };
        ...
        float Reward(StateData originalState, ActionKey action, StateData newState)
        {
            var reward = 0f;
            {
                reward += new AI.Planner.Actions.Match3Plan.CustomSwapReward().RewardModifier( originalState, action, newState);
            }
